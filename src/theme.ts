...
export function getCalculatedColorSheme() {
    ...
    if (colorSheme === 'system')
        colorSheme = nativeTheme.shouldUseDarkColors ? 'dark' : 'light';
    return colorSheme;
}
...
const getCssFilesWithThemes = (
    dirPath: string
): Promise<CssFileWithTheme[]> => {
...
export function getThemeList(): Promise<CssFileWithTheme[]> {
    const themesDir = path.resolve(
        path.join(__dirname, '../public/css/themes')
    );
